...
def main() -> int:
    ...
    all_ok = _ok("aucun hit semantique en mode safe", len(result.semantic_hits) == 0) and all_ok
    all_ok = _ok("reponse non vide", bool(result.answer.strip())) and all_ok

    answer_lower = result.answer.lower()
    relevant_answer = ("mode safe" in answer_lower) or ("28" in answer_lower) or ("gateau" in answer_lower)
